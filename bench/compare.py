...
def compare_results(current: dict[str, Any], baseline: dict[str, Any]) -> tuple[bool, list[str]]:
    """
...
def print_comparison(current: dict[str, Any], baseline: dict[str, Any], issues: list[str]) -> None:
...
def main():
    ...
    parser.add_argument(
        "--set-baseline", action="store_true", help="Copy current results to baseline"
    )
    parser.add_argument(
        "--quiet", action="store_true", help="Only print if regressions are detected"
    )
